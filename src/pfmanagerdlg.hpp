...
public:
    ...
    {
       setupUi(this);
    QString val = trueos::Utils::getConfFileValue( "/etc/rc.conf", "firewall_enable=");
    if(val.isEmpty()){ val = trueos::Utils::getConfFileValue("/etc/rc.conf.trueos","firewall_enable="); }
    //qDebug() << val;
