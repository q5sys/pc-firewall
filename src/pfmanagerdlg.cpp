...
void PFManagerDlg::enableClicked ( void )
{
    ...
    if (firewallRunning)
    {
        ...
        {
            //_firewall.enable();
	    trueos::Utils::setConfFileValue( "/etc/rc.conf", "firewall_enable=", "firewall_enable=\"YES\"", -1);
		...
		trueos::Utils::setConfFileValue( "/etc/rc.conf", "firewall_enable=", "firewall_enable=\"NO\"", -1);
            } else {
...
void PFManagerDlg::restoreClicked ( void )
{
    ...
    system("sh /usr/local/share/trueos/scripts/reset-firewall");
...
void PFManagerDlg::SaveOpenPorts(){
  ...
  if(firewallRunning){ system("sh /usr/local/share/trueos/scripts/reset-firewall"); }
}
